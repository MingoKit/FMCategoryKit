...
@implementation UIView (FMXibStyle)
@dynamic roundTop, roundLeft, roundBottom, borderWidth, borderColor, cornerRadius, shadowColor, shadowRadius, shadowOffset, shadowOpacity, themeGradientEnable, gradientStyle, gradientStyleEnum, gradientAColor, gradientBColor, backgroundShadowLayer, gradientLayer, bColor, bWidth, clipRadius, radius;

+(void)load{
    NSArray *arr = @[@"setHidden:" ,@"setAlpha:", @"layoutSubviews", @"removeFromSuperview", @"setFrame:"];
    for (NSString *str in arr) {
        NSString *new_str = [@"fm_" stringByAppendingString:str];
...
- (void)fm_setFrame:(CGRect)frame
{
    [self fm_setFrame:frame];
    if (self.backgroundShadowLayer) {
...
- (void)fm_setAlpha:(CGFloat)alpha{
    [self fm_setAlpha:alpha];
...
- (void)fm_setHidden:(BOOL)hidden
{
    [self fm_setHidden:hidden];
    if (self.backgroundShadowLayer) {
